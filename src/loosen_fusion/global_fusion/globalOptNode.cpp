...
ros::Publisher pub_global_odometry, pub_global_path, pub_car,pub_gnss_path;
nav_msgs::Path *global_path;
nav_msgs::Path *gnss_path;
double last_vio_t = -1;
double last_lio_t = -1;
std::queue<sensor_msgs::NavSatFixConstPtr> gpsQueue;
std::queue<sensor_msgs::NavSatFixConstPtr> gpsQueueforLIO;
std::mutex m_buf,m_buf2;
...
void lio_callback(const nav_msgs::Odometry::ConstPtr &pose_msg)
{
    ...
    globalEstimator.inputLidarOdom(t, lio_t, lio_q);

    m_buf2.lock();
    while(!gpsQueueforLIO.empty())
    {
        sensor_msgs::NavSatFixConstPtr GPS_msg = gpsQueueforLIO.front();
...
void vio_callback(const nav_msgs::Odometry::ConstPtr &pose_msg)
{
    ...
    pub_global_path.publish(*global_path); //发布融合后的轨迹

    gnss_path = &globalEstimator.gnss_path;
    gnss_path->header = odometry.header;
    pub_gnss_path.publish(*gnss_path); //发布GNSS轨迹
...
int main(int argc, char **argv)
{
    ...
    ROS_INFO("\033[1;32m----> global_fusion_node Started.\033[0m");
    
    // //读取原点经纬度坐标
    // double lat,lon,alt;
    // nh.param<double>("fusion_initial_GPS_longitude", lat, 124.1);
    // nh.param<double>("fusion_initial_GPS_latitude", lon, 24.1);
    // nh.param<double>("fusion_initial_GPS_altitude",alt, 10);
    
    // ori_lat = lat;
    // ori_lon = lon;
    // ori_alt = alt;
    // std::cout<<ori_lat<<","<<ori_lon<<std::endl;
    ...
    ros::Subscriber sub_vio = n.subscribe("/lviorf/vins/odometry/odometry", 100, vio_callback);
    // ros::Subscriber sub_lio = n.subscribe("/lviorf/mapping/odometry", 100, lio_callback);
    pub_global_path = n.advertise<nav_msgs::Path>("global_path", 100); //融合后的轨迹
    pub_global_odometry = n.advertise<nav_msgs::Odometry>("global_odometry", 100);
    pub_car = n.advertise<visualization_msgs::MarkerArray>("car_model", 1000);

    pub_gnss_path = n.advertise<nav_msgs::Path>("gnss_path", 100); //gnss轨迹
