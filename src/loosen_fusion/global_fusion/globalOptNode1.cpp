...
double last_vio_t = -1;
std::queue<nav_msgs::OdometryConstPtr> gpsQueue;
std::mutex m_buf;
...
void vio_callback(const nav_msgs::Odometry::ConstPtr &pose_msg)
{
    ...
    {
        nav_msgs::OdometryConstPtr GPS_msg = gpsQueue.front();
        ...
        if(gps_t >= t - 0.01 && gps_t <= t + 0.01)
        {
            ...
            globalEstimator.inputGPS(t, GPS_msg);
            gpsQueue.pop();
            break;
        }
...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber sub_GPS = n.subscribe("/gnss_calibrated", 100, GPS_callback);
