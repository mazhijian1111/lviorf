...
public:
    LidarFusion() : tf_listener_(tf_buffer_)
    {
        ...
        {                                                                                     // 消息转换，区分frame id
            ...
            subs_.push_back(nh.subscribe<PointCloud2>("/lidar_back_left_helios32", 1, boost::bind(&LidarFusion::cloudCallback, this, _1, 1))); // 创建订阅者
            // subs_.push_back(nh.subscribe<PointCloud2>("/lidar_back_up_helios32", 1, boost::bind(&LidarFusion::cloudCallback, this, _1, 2))); // 创建订阅者
            subs_.push_back(nh.subscribe<PointCloud2>("/lidar_front_right_helios32", 1,boost::bind(&LidarFusion::cloudCallback, this, _1, 2))); // 创建订阅者
            // subs_.push_back(nh.subscribe<PointCloud2>("/lidar_front_up_helios32", 1,boost::bind(&LidarFusion::cloudCallback, this, _1, 4))); // 创建订阅者
        ...
        // 创建同步策略
        sync_.reset(new Synchronizer<SyncPolicy>(SyncPolicy(2),sub_back_left_, sub_front_right_));
        sync_->registerCallback(boost::bind(&LidarFusion::fusionCallback, this, _1, _2));
...
private:
    void fusionCallback(const PointCloud2ConstPtr &cloud1, const PointCloud2ConstPtr &cloud2)
    {
        std::cout<<"进入fusionCallback"<<std::endl;
        try
        {
            // 合并点云
            if (lidar_type == 4)
    ...
    {
        std::cout<<"进入ID为"<<radar_idx<<"的雷达回调函数"<<std::endl;
        // 创建消息副本
